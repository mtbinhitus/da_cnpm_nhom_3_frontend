        ...
        <div>
            <Stack py={2} justifyContent="center" spacing={2}>
                ...
                {props.listExam.map((value, item) => (
                    <Stack key={item} direction="row" pt={1} justifyContent="flex-start" container spacing={2} mt={2}>
                        <div className="questionNumber">
                            <strong>{item + 1}</strong>
                        </div>
                        <div className="option-list">
                            {value.question.length > 0 && <Typography variant="subtitle1">{value.question}</Typography>}
