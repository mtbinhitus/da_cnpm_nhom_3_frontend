...
import { Grid, Container, Stack, Typography, RadioGroup, FormControlLabel, Radio, Alert } from '@mui/material';
...
export default function QuestionBox(props) {
    ...
    if (props.data === null) return <Alert severity="error">Exam information is not available!</Alert>;
    if (props.data.questions[0] === undefined)
        return <Alert severity="error">Exam information is not available!</Alert>;
    ...
    const handleChange = (event, index) => {
        ...
        props.fillAnswerSheet({ id: parseInt(event.target.name, 10), answer: event.target.value });
    };
        ...
        <Grid mt={2} container spacing={2} alignItems="center" justify="center">
            ...
            >
                {props.data.questions.map((item, index) => (
                    <Stack key={item.id} pt={1} justifyContent="flex-start" spacing={2} mt={2}>
